...
def get_input():
    ...
    bold = lambda x: colored(f"[{x}]", attrs=["bold"])

    data_dir = input(f"Path to data directory? {bold(DATA_DIR)}: ")
    if data_dir:
        DATA_DIR = data_dir

    out_dir = input(f"Where to output files? {bold(OUT_DIR)}: ")
    if out_dir:
        OUT_DIR = out_dir

    key_spreadsheet = input(f"Name of the treatment sheet? {bold(KEY_SPREADSHEET)}: ")
    if key_spreadsheet:
        KEY_SPREADSHEET = key_spreadsheet

    data_sheet_name = input(
        f"Name of data tab within data sheets? {bold(DATA_SHEET_NAME)}: "
    )
    if data_sheet_name:
